...
def gibbs():
    ...
    while curr_position_list != prev_position_list:
        ...
        for sequence_i, position in enumerate(curr_position_list):
            ...
            curr_position_list[sequence_i] = find_best_index(sequence_list[sequence_i], pwm)
...
def main():
    ...
    if not os.path.exists('outcomes_old/{}'.format(data_folder)):
        os.makedirs('outcomes_old/{}'.format(data_folder))
    ...
    for num in range(10):
        ...
        # Generate files
        if not os.path.exists('outcomes_old/{}/{}'.format(data_folder, str(num))):
            os.makedirs('outcomes_old/{}/{}'.format(data_folder, str(num)))

        with open('outcomes_old/{}/{}/predictedmotif.txt'.format(data_folder, str(num)), 'w') as f:
        ...
        with open('outcomes_old/{}/{}/predictedsites.txt'.format(data_folder, str(num)), 'w') as f:
            for l in curr_position_list:
                f.write(str(l) + '\n')

        runtime = (time.time() - run_start)
        with open('outcomes_old/{}/{}/runtime.txt'.format(data_folder, str(num)), 'w') as f:
            f.write(str(runtime))
